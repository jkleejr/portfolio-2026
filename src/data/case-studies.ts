    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Narrate - Each group is sent to Gemini TTS, comes back as raw audio data, and is wrapped in a WAV and cached on disk.",
          "Display - Highlighted sentences are estimated from audio length and character count, and re-syncs at the end of every group."
        ],
      },
      {
        type: "text",
        text: "Because Gemini only returns the audio, the app estimates timings to highlight the current sentence. First, audio arrives as a list of 24,000 numbers per second, so the total seconds in a group can be calculated (total numbers / 24,000 = total seconds). At this point the app knows the total seconds, the number of sentences, but not how long each sentence could take. It splits the time by text (each sentence characters / group total characters = its % of the text). Then (% of the text x total seconds = how long each sentence could take). As the group plays, the app tracks how many seconds have passed to guess the current sentence.",
      },

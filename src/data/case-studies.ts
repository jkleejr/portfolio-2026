...
export const caseStudies: Record<string, CaseStudy> = {
  ...
  "paper-reader": {
    ...
    blocks: [
      ...
      {
        type: "text",
        text: "Since Gemini returns the audio and nothing else, the app estimates timings to highlight the current sentence. First, audio arrives as a list of 24,000 numbers per second, so the total seconds in an audio clip can be calculated based on the total numbers in the list (total numbers / 24,000 = total seconds). At this point the app knows the total seconds, the number of sentences, but not how long each sentence could take. So it splits the time by text (each sentence's characters / the group's total characters = its % of the text). Then (% of the text x total seconds = how long each sentence could take). Now the app can guess which sentence is currently being played based on the durations for each sentence in a clip. As the clip plays, the app also tracks how many sentences have passed to estimate the current sentence.",

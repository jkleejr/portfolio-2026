...
import { entries, type EntryImage } from "@/data/projects";
...
function Thumbnail({ image, href }: { image: EntryImage; href?: string }) {
  if (image.src) {
    ...
    const img = (
      <img
        src={image.src}
        alt={image.alt}
        className="h-[120px] w-[120px] rounded-lg border border-foreground/10 object-cover"
    ...
    if (href) {
      return (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="block transition-transform duration-200 ease-out hover:scale-105"
        >
          {img}
        </a>
      );
  ...
  return (
    <div
      className="h-[120px] w-[120px] rounded-lg border border-foreground/10 bg-foreground/[0.02]"
                ...
                <div className="mt-5 flex flex-wrap gap-4">
                  {entry.images.map((image, i) => (
                    <Thumbnail
                      key={i}
                      image={image}
                      href={image.href ?? entry.titleHref ?? entry.link?.href}
                    />
                  ))}
                </div>
              )}

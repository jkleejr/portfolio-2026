...
// Bits). Pressing it again turns the trail off, as does Escape. It is a bare
...
export function PhotoGalleryButton() {
  ...
  return (
    <button
      ...
      aria-pressed={active}
      // Bare at rest, and inverted both under the pointer and for as long as
      // it is on: the icon drops out of a filled disc. The hover is what says
      // it can be pressed; the pressed state is the switch saying so before
      // you have moved the pointer far enough to see a photo. Hovering it
      // while it runs changes nothing, which is right — it is already there.
      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-foreground transition duration-200 ease-out hover:bg-foreground hover:text-background aria-pressed:bg-foreground aria-pressed:text-background"
    >
      <GalleryIcon />
    </button>
  );
